...
\begin{frame}{The one-slide summary}
  \begin{block}{Problem}
    An agentic assistant for public services can return a fluent, confident answer built on a
    service response that \hl{timed out, arrived incomplete, or came back corrupted}.
  ...
  \begin{block}{Result (100 boundary calls, five injection rates)}
    ...
    matching the closed form $0.25 + 2 \times 0.25 \times 0.75 = 0.625$. That rate is a
    property of the \hl{response schema} rather than an implementation defect. Inference
    costs about \$0.05 per session.
  \end{block}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      ...
      \begin{block}{Consequence}
        The arithmetic is trivial. What costs the citizen time is \hl{coordinating the
        procedures and interpreting the rules}, and that is the part an agent can absorb.
      \end{block}
    \end{column}
...
\begin{frame}{Two gaps}
  ...
  \vfill
  \centering
  \hl{Both gaps concern the boundary between a probabilistic model and a deterministic
  service contract.}
\end{frame}
...
\begin{frame}{A concrete request}
  ...
  \vfill
  \centering
  \hl{The paper measures how often that happens, and what governs the number.}
\end{frame}
...
\begin{frame}{Fault injection results}
  ...
  \vfill
  \begin{itemize}
    \item The measured rate sits on the predicted \risk{0.625}. Raising $\eta$ raises the
...
\begin{frame}[noframenumbering]{Backup: where the work sits}
  \begin{columns}[T]
    \begin{column}{0.31\textwidth}
      ...
      unlike open-ended conversation. That matters when the service is regulated.}
    \end{column}
    \begin{column}{0.31\textwidth}
