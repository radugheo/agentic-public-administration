...
\begin{frame}{The one-slide summary}
  ...
  \begin{block}{Result (100 boundary calls, five injection rates)}
    ...
    the shape of the \hl{response schema}; no amount of debugging the implementation will
  ...
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      ...
      \begin{block}{Consequence}
        ...
        procedures and interpreting the rules}, and that is what the agent takes over.
      \end{block}
    \end{column}
...
\begin{frame}{Two gaps}
  ...
  \begin{block}{2. Boundary faults are invisible to agent control flow}
    Fault injection has reached LLM agents, but in generic or synthetic testbeds. Existing work
    stops there, leaving the fraction of faults that propagate silently unmeasured for a
    regulated public service, and unconnected to the structure of the response schema.
    Both gaps lie on the \hl{boundary between a probabilistic model and a deterministic
    service contract}.
  \end{block}
\end{frame}
...
\begin{frame}{A concrete request}
  \begin{columns}[T]
    ...
    \begin{column}{0.47\textwidth}
      ...
      Each of those steps crosses a service boundary. The SPV call can
      \begin{itemize}\small
        \item time out
      ...
      We measure how often that occurs, and what determines the rate.
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{block}{Infrastructure: UiPath Python SDK}
        \small Model gateway, retrieval indices, storage buckets, queues and human review tasks.
...
\begin{frame}{One entry point, confidence-gated routing}
  \begin{center}
...
\begin{frame}{The fault injector}
  \begin{columns}[T]
    ...
    \begin{column}{0.50\textwidth}
      ...
      uniformly, and each accounts for a quarter of injected faults. The injector is a separate
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{Prototype}
        ...
        Every government call goes through the mocked boundary layer; no live network
        traffic is required.
      \end{block}
      \begin{block}{Runs}
...
\begin{frame}{Conclusion}
  \begin{block}{What the evaluation shows}
    \begin{itemize}\small
      ...
      \item Inference cost stays affordable at institutional volumes.
    \end{itemize}
  \end{block}
