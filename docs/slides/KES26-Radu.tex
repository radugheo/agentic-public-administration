...
\begin{frame}{The fragmentation problem}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      ...
      \begin{itemize}
        \item \textbf{SPV}, \emph{Spa\c{t}iul Privat Virtual} (Virtual Private Space):
          filing declarations, consulting your fiscal record
        \item \textbf{Ghi\c{s}eul.ro}, from \emph{ghi\c{s}eu} (service counter):
      ...
      \begin{block}{Where the burden sits}
        \small Computing the amounts is simple arithmetic. The effort goes into \hl{which
        declaration applies, under which deadline, on which portal}, and the rules that settle
        each.
      \end{block}
    \end{column}
...
\begin{frame}{Gaps in existing work}
...
\begin{frame}{What this paper does}
  ...
  \begin{block}{Break}
    ...
    19 boundary operations, four chaos operators (timeout, delay, partial response, corruption),
    a configurable rate, replayable from a seed.
  \end{block}
  \begin{block}{Measure}
  ...
  \begin{columns}[T]
    \begin{column}{0.46\textwidth}
      \begin{block}{Mechanism}
        \small A decorator wraps each mocked government boundary. \hl{One Bernoulli draw per
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{Prototype}
        \small Python 3.11, LangGraph 0.2, GPT-4o through the UiPath LLM Gateway, retrieval
        through UiPath Context Grounding. Every government call goes through the mocked boundary
        layer, so no live portal traffic is required.
      \end{block}
      \begin{block}{Runs}
        \small $N = 100$ boundary calls, each an independent trial with at most one injected
        fault, sampled across the 8 portal-facing mock tools by call frequency in the five
...
\begin{frame}{What remains to be tested}
  ...
  \vfill
  \begin{enumerate}\small
    \item \textbf{Output contracts.} Validity conditions evaluated on the assembled response
      before it reaches the citizen: a submission identifier that must be present and well
      formed, a payment code that must match its shape. The fix the analysis points to; what
      it recovers is an open number.
    \item \textbf{Other agent architectures.} A \hl{ReAct loop} choosing freely among the shared
      ...
      untested.
    \item \textbf{Guardrails.} Schema validators and policy checks at the boundary itself,
      ahead of agent logic, catching what a branching predicate structurally cannot.
    ...
    \item \textbf{Realistic conditions.} Live portals in place of mocks, and faults compounding
      across a full session rather than one call at a time.
  \end{enumerate}
\end{frame}
